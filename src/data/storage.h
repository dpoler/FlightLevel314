...
struct UserConfig {
    ...
    int trail_style;         // 0=line, 1=dots -- unused (dead field, kept for config compat)

    // Display sleep / brightness / screensaver fields — kept in config for
    // JSON compatibility; no active screensaver UI on Pi today.
    ...
    int display_blank_after_min;  // idle minutes before blank; 0 = never (default 0)
    bool screensaver_enabled;     // reserved
    bool screensaver_drift;       // reserved
...
// Wipe persisted config (deletes config.json on Pi). Does not reload
// g_config — caller should reload or restart as appropriate.
