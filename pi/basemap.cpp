...
bool basemap_rebuild_current(void) {
    ...
    std::string path;
    {
        ...
        if (g_req_w <= 0 || g_req_h <= 0 || g_req_br <= 0) {
            platform_log_info("Basemap: rebuild current — no active map request\n");
            return false;
        }
        lat = g_req_lat;
        lon = g_req_lon;
        ...
        // Drop the drawn buffer so a blank tile isn't sticky while we refetch.
        g_front = BasemapSlot{};
        g_inbox = BasemapSlot{};
        g_inbox_ready = false;
        g_unavailable = false;
    ...
    if (unlink(path.c_str()) == 0) {
        platform_log_info("Basemap: rebuild current — removed %s\n", path.c_str());
    } else {
        platform_log_info("Basemap: rebuild current — no cache file (will refetch) %s\n",
                          path.c_str());
    }
