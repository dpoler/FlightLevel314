...
void pi_input_init(lv_display_t *disp) {
    ...
    if (!dev) {
        platform_log_warn("Input: no libinput TOUCH device found — taps will not work\n");
        return;
    }

    platform_log_info("Input: using touch device %s\n", dev);
    lv_indev_t *indev = lv_libinput_create(LV_INDEV_TYPE_POINTER, dev);
    if (!indev) {
        platform_log_warn("Input: lv_libinput_create failed for %s\n", dev);
        return;
    }
